...
- **UPDATE 2026-06-09 evening:** the toggle is now REAL — raw allow/ignore fields collapse behind the chips when patterns match a preset, `Ctrl+R` toggles them (bare-letter `a` can never fire while an Input has focus — documented Textual-medium deviation), chips are clickable and fill the raw inputs, and the chip highlight derives from the actual values. The dead `o Override revision` footer hint was removed.
...
- **Refactor delivered:** what-it-does subtitle; a `ValidationCard` slot that starts NEUTRAL and renders REAL probe results as you type (UPDATE 2026-06-09 evening: `inspect_venv` engine fn + agent method + optional `probe` ctor kwarg wired from app.py; ok=green with detected vllm/torch/python versions + version auto-fill, failure=red with the named reason; never fabricates success); Field-wrapped inputs; copy `Checkbox` + tradeoff helper; WILL-DO preview; `KeyHintBar`. Submission via `on_input_submitted` → `action_submit` (added). Focus on `#adopt-build-venv-path` on mount.
...
- **Phase 5 — Dashboard log classification `60:2` (screenshot #7) — ✅ COMPLETED** (`f993be5`). `display_level_for_line` + `BENIGN` dim level. CORRECTION (2026-06-09 evening review): the claim "dashboard chrome was already v1-styled" was WRONG — the cryptic `▣`/`M` header glyph segments remained (they only render when a target is connected AND a config is loaded, so the idle render looked clean). Fixed in the evening pass: labeled `build:`/`model:` segments + sidebar config names ellipsize instead of wrapping mid-word. PhaseStepper / GPU card / compact security notice remain descoped.
