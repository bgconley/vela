...
> Auto-maintained by OpenWolf. Last scanned: 2026-07-12T06:53:50.290Z
...
- `append_bug283.py` (~666 tok)
- `append_bug284.py` (~922 tok)
...
- `dedupe_buglog.py` — Dedupe .wolf/buglog.json (reviewer follow-up, Task 5.2). (~528 tok)
...
- `repro_daemon_leak.py` — Reproduce the test-suite daemon leak (Phase-8 commit 0.4). (~1090 tok)
- `repro_m5_sha_pin_gating.py` — Independent repro for M5 (5.8): --commit-sha pins still detect gating; sha is trusted. (~2447 tok)
- `repro_m6_active_build_integrity.py` — Independent repro for M6 (5.9): tampered ACTIVE build blocks venv launches, not docker. (~1940 tok)
- `repro_p7_keepalive.py` — Reviewer independent repro — Phase 7 commit 0 (0.1): keepalive preview-window guard. (~2426 tok)
- `repro_p7_precedence.sh` — Reviewer independent repro — 7.3: target-resolution precedence chain, real CLI, (~902 tok)
- `repro_p7_runs_scrub.py` — Reviewer independent repro — 7.2: `vela runs list` scrub with a HOSTILE payload. (~1103 tok)
- `repro_p8_daemon_escalation.py` — Independent Phase-8 verification (b): daemon-stop escalation, both directions. (~1341 tok)
- `repro_p8_freshbox.sh` — Independent Phase-8 verification (a): fresh-box walk. (~444 tok)
- `repro_p8_readme_audit.sh` — Independent Phase-8 verification (d): README command audit. (~885 tok)
- `reviewer_45_probe.py` — Reviewer probe (Task 4.5): header invariants at boundary widths + busy-verb transient. (~994 tok)
- `reviewer_56_57_repro.py` — Reviewer independent repros for Tasks 5.6 (M1) + 5.7 (M2). (~6058 tok)
...
- `test_review_upsert_repro.py` — Reviewer repro: does the common pin -> cache-refresh -> re-pin flow upsert or mint? (~1378 tok)
- `update_bug240.py` — Idempotently finalize bug-240 (docker pull timeout) in .wolf/buglog.json. (~713 tok)
...
- `wolf_review_56_57.py` — Reviewer bookkeeping for Tasks 5.6+5.7 review (idempotent, unstaged .wolf edits). (~1745 tok)
- `wolf_review_p7.py` — Reviewer bookkeeping for the Phase-7 gate review (idempotent, unstaged .wolf edits). (~1602 tok)
...
- `CHANGELOG.md` — Change log (~455 tok)
- `CLAUDE.md` — OpenWolf (~57 tok)
- `LICENSE` — Project license (~286 tok)
- `pyproject.toml` — Vela is a phase-aware terminal TUI for launching vLLM servers from named configs. (~514 tok)
- `README.md` — Project documentation (~2867 tok)
...
- `vela-docker-runtime-examples-v1.md` — Vela — Docker Runtime — Worked Examples (DK4 anchor): Blackbird Qwen3.6 27B FP8 & BF16 (~3771 tok)
...
- `vela-session-context-2026-06-10-gpu-validation.md` — Session context — 2026-06-10 — journey punchlist done, GPU validation in flight (~1169 tok)
...
- `vela-tui-overhaul-implementation-plan-v1.md` — Vela TUI Overhaul — Implementation Plan (v1) (~4215 tok)
...
- `vela-v1-completion-audit-2026-06-07.md` — Vela v1 Completion Audit - 2026-06-07 (~1965 tok)
- `vela-v1-completion-punchlist.md` — Vela — v1 Completion Punchlist & Coder Handoff (~10796 tok)
- `vllm-agent-architecture-review-punchlist-v2.md` — vLLM Agent Architecture — Implementation Review Punch List **v2** (~3625 tok)
- `vllm-agent-architecture-review-punchlist-v3.md` — vLLM Agent Architecture — Implementation Review Punch List **v3** (~2134 tok)
- `vllm-agent-architecture-review-punchlist-v4.md` — vLLM Agent Architecture — Implementation Review Punch List **v4** (~2295 tok)
- `vllm-agent-architecture-review-punchlist-v5.md` — vLLM Agent Architecture — Implementation Review Punch List **v5** (v1 sign-off checklist) (~1942 tok)
- `vllm-agent-architecture-review-punchlist.md` — vLLM Agent Architecture — Implementation Review Punch List (~2840 tok)
- `vllm-agent-architecture-spec-v1.md` — Vela — Agent/Controller Architecture (Remote Execution) — Specification & Implementation Plan (v1) (~14415 tok)
- `vllm-build-management-spec-v1.md` — vLLM Build Management — Feature Specification & Implementation Plan (v1) (~12483 tok)
- `vllm-full-repo-review-findings-v1.md` — vLLM Loader — Full-Repo Review & Findings (cross-spec) **v1** (~5199 tok)
...
- `vllm-tui-loader-spec-v2-CANONICAL.md` — vLLM TUI Model Loader — Canonical Specification & Implementation Plan (v2) (~13084 tok)

## .claude/

- `settings.json` (~441 tok)
- `settings.local.json` — Declares bfloat16 (~551 tok)
...
- `ci.yml` — CI: CI (~274 tok)
- `remote-validation.yml` — CI: Remote Validation (~2027 tok)
...
- `2026-06-04-blackbird-fake-child-c37ca7d.md` — vLLM Loader Remote Validation (~2518 tok)
- `2026-06-04-blackbird-real-build-model-qwen-54c612e.md` — vLLM Loader Remote Validation (~13742 tok)
- `2026-06-04-p620-blackbird-b085610-build-model-resume.md` — vLLM Loader Remote Validation (~13548 tok)
- `2026-06-04-p620-blackbird-b085610-qwen-smoke.md` — vLLM Loader Remote Validation (~2817 tok)
- `2026-06-04-p620-blackbird-eb2a116-remote-validation.md` — vLLM Loader Remote Validation (~13724 tok)
- `2026-06-04-p620-blackbird-smoke.md` — P620 to Blackbird Remote Smoke - 2026-06-04 (~1751 tok)
- `2026-06-04T20-04-41Z-bgconley-10.25.0.50-qwen36-27b-fp8-kvfp8-rp6000-blackbird-remote-validation.md` — vLLM Loader Remote Validation (~13891 tok)
- `2026-06-04T20-34-19Z-bgconley-10.25.0.50-remote-validation.md` — vLLM Loader Remote Validation (~2594 tok)
- `2026-06-06-p620-blackbird-native-docker-bf16-9b107b4.md` — P620 -> Blackbird Native Docker BF16 Validation (~435 tok)
- `2026-06-06-p620-blackbird-native-docker-fp8-d67b3a6.md` — P620 -> Blackbird Native Docker FP8 Validation (~466 tok)
- `2026-06-07-p620-blackbird-native-docker-bf16-63b73b3.md` — Vela Remote Validation (~3018 tok)
- `2026-06-07-p620-blackbird-native-docker-fp8-63b73b3.md` — Vela Remote Validation (~3468 tok)
- `2026-06-07-p620-blackbird-native-docker-fp8-cb1eed8-retry2.md` — Vela Remote Validation (~3195 tok)
- `2026-06-07-p620-blackbird-native-docker-fp8-cb1eed8.md` — Vela Remote Validation (~2772 tok)
- `2026-06-07-p620-blackbird-native-docker-fp8-e3a5c6d.md` — Vela Remote Validation (~18855 tok)
- `2026-06-07T06-35-55Z-bgconley-10.25.0.50-qwen36-27b-bf16-rp6000-blackbird-remote-validation.md` — Vela Remote Validation (~3095 tok)
- `2026-06-07T06-56-01Z-bgconley-10.25.0.50-qwen36-27b-bf16-rp6000-blackbird-remote-validation.md` — Vela Remote Validation (~3310 tok)
- `2026-06-07T07-01-42Z-bgconley-10.25.0.50-qwen36-27b-fp8-kvfp8-rp6000-blackbird-remote-validation.md` — Vela Remote Validation (~3276 tok)
- `2026-06-07T07-38-08Z-bgconley-10.25.0.50-remote-validation.md` — Vela Remote Validation (~2590 tok)
- `2026-06-07T07-49-52Z-bgconley-10.25.0.50-qwen36-27b-fp8-kvfp8-rp6000-blackbird-remote-validation.md` — Vela Remote Validation (~3207 tok)
- `2026-06-07T07-54-32Z-bgconley-10.25.0.50-qwen36-27b-bf16-rp6000-blackbird-remote-validation.md` — Vela Remote Validation (~3134 tok)
- `2026-06-07T08-07-22Z-bootstrap-and-hf-blackwell-recipe-audit.md` — Bootstrap And HF Blackwell Recipe Audit - 2026-06-07 (~1035 tok)
- `2026-06-10T07-02-37Z-bgconley-10.25.0.51-qwen36-27b-bf16-rp6000-blackbird-remote-validation.md` — Vela Remote Validation (~246 tok)
- `2026-06-10T07-14-46Z-bgconley-10.25.0.51-qwen36-27b-bf16-rp6000-blackbird-remote-validation.md` — Vela Remote Validation (~10355 tok)
- `2026-06-10T07-26-04Z-bgconley-10.25.0.51-qwen36-27b-bf16-rp6000-blackbird-remote-validation.md` — Vela Remote Validation (~9586 tok)
- `2026-06-10T07-47-58Z-bgconley-10.25.0.51-qwen36-27b-bf16-rp6000-blackbird-remote-validation.md` — Vela Remote Validation (~3307 tok)

## artifacts/tui-screenshots/configs/

- `bad-config.yaml` (~6 tok)
- `fake-child.yaml` (~66 tok)
- `lan-warning.yaml` (~29 tok)

## configs/

- `fake-child.yaml` (~91 tok)
- `qwen3-32b-fp8-62001.yaml` (~210 tok)
- `qwen36-27b-bf16-rp6000-blackbird.yaml` (~705 tok)
- `qwen36-27b-fp8-kvfp8-rp6000-blackbird.yaml` (~906 tok)
- `real-vllm.example.yaml` (~127 tok)
- `tiny-random-llama-detached-blackbird.yaml` (~530 tok)
...
- `backend_evidence_check.py` — from: parse_args, validate_backend_evidence, main (~2945 tok)
- `blackbird_qwen36_bf16_vllm_foreground.sh` (~1422 tok)
- `blackbird_qwen36_vllm_foreground.sh` (~2467 tok)
- `fake_vllm_child.py` (~63 tok)
- `gated_model_auth_check.py` — parse_args, main (~1050 tok)
- `gen_tui_docs.py` — Generate ``docs/tui.md`` from the TUI's declared key bindings (drift-proof). (~1342 tok)
...
- `run_remote_tests.sh` (~7548 tok)
- `smoke_fake_child.sh` (~49 tok)

## src/vela/

- `__init__.py` — Vela package. (~23 tok)
- `__main__.py` (~29 tok)
- `cli.py` — API router (~39030 tok)
...
- `daemon.py` — from: close, wait_closed, default_agent_runtime_dir, default_agent_socket_path + 8 more (~5922 tok)
- `local.py` — Declares BuildInstallRequest (~63442 tok)
...
- `loader.py` — ValidConfig: by_name, discover_config_dirs, load_registry, first_existing_dir + 1 more (~1229 tok)
- `schema.py` — Pydantic: DockerConfig (~2235 tok)
- `targets.py` — from: name_must_not_be_empty, agent_command_from_shell_string, validate_transport_fields, targets + (~2501 tok)

## src/vela/engine/

- `__init__.py` — Engine, process, log, and vLLM-version adapter code. (~17 tok)
- `build_registry.py` — from: payload, default_builds_root, active_build_id, resolve_build_handoff + 8 more (~12538 tok)
...
- `docker_runtime.py` — DockerRunCommand: build_docker_run, prepare_docker_image, pull_docker_image, inspect_docker_image + (~3429 tok)
- `ids.py` — mint_ulid (~132 tok)
- `job_phases.py` — Declares import (~126 tok)
- `log_sink.py` — from: feed, close, rotate_to, scrub + 5 more (~1715 tok)
- `model_registry.py` — from: metadata, env_contribution, default_models_registry_path, default_hf_hub_cache_dir + 3 more (~26668 tok)
- `phases.py` — Phase: feed_line, health_ready, health_failed, health_error + 2 more (~1204 tok)
- `preflight.py` — from: check_launch_preflight, missing_local_model_path, parallel_world_size_mismatch, occupied_port_ (~2762 tok)
- `process_manager.py` — from: read_loop, stop, kill, start_attached + 1 more (~3673 tok)
- `profile.py` — VllmDefaults: flag_for, without_flags, soft_validate, bundled_profile + 6 more (~3120 tok)
- `redaction.py` — scrub_text (~153 tok)
- `run_pruning.py` — Retention for flat-file run records under runs directories. (~1220 tok)
- `sidecar.py` — TrackedProcessMismatch: from_active_log, rotate_to, write_atomic, to_json + 12 more (~4720 tok)
- `supervisor.py` — _DrainOnlySink: feed, close, rotate_to, emit + 4 more (~6427 tok)

## src/vela/monitoring/

- `__init__.py` — Health and GPU monitoring. (~10 tok)
- `gpu.py` — GpuSample: sample_gpus, parse_nvidia_smi_csv, parse_cuda_visible_devices, apply_cuda_visible_devices (~1606 tok)
- `health.py` — import: probe_host_for, check_once, probe_loop (~1592 tok)

## src/vela/transport/

- `__init__.py` — Controller-to-target transports. (~12 tok)
- `client.py` — TargetClient: handshake_params, subscription_event_id, event_matches_subscription, agent_error_event + 6 more (~762 tok)
- `factory.py` — target_client_for_config, ssh_command_for_target (~3838 tok)
- `inprocess.py` — InProcessTargetClient: connected, connect, disconnect, call + 3 more (~981 tok)
- `ndjson.py` — NdjsonFrameError: encode_frame, decode_frame (~288 tok)
- `rpc_errors.py` — rpc_error_payload, target_call_error_from_rpc_payload (~754 tok)
- `socket.py` — UnixSocketTargetClient: connected, connect, disconnect, call + 3 more (~3073 tok)
- `ssh_bootstrap.py` — from: install_ssh_agent (~885 tok)
- `ssh_discovery.py` — URL configuration (~2116 tok)
- `ssh_setup.py` — from: setup_ssh_key (~532 tok)
- `subprocess.py` — SubprocessTargetClient: connected, connect, disconnect, call + 3 more (~4014 tok)
...
- `.DS_Store` (~1640 tok)
- `conftest.py` — scaled_timeout, isolated_vela_state, config_dir, write_yaml (~1363 tok)
- `test_adopt_build_screen.py` — Headless tests for the refactored AdoptBuildScreen (Mac-safe; no GPU/vLLM). (~1793 tok)
- `test_agent_client.py` — LegacyCapabilityAgent: test_target_client_requires_lifecycle_capabilities, test_agent_connect_bridge (~140558 tok)
- `test_agent_daemon.py` — test_stale_local_daemon_banner_flags_version_drift, test_stale_local_daemon_banner_flags_newer_daemo (~9517 tok)
...
- `test_branding.py` — test_project_is_branded_as_vela, test_default_product_paths_and_agent_command_use_vela, test_module_ (~952 tok)
...
- `test_cli_run.py` — FakeTargetsRegistry: test_debug_mode_enables_textual_debug_and_devtools, test_every_cli_command_and_ (~64229 tok)
- `test_command_builder.py` — cfg, test_exact_argv_env_for_serve_entrypoint, test_revision_pin_is_emitted_for_standalone_model_han (~5970 tok)
- `test_config_loader.py` — test_valid_config_loads, test_invalid_yaml_or_schema_error_is_retained, test_duplicate_names_are_det (~2842 tok)
- `test_create_build_screen.py` — Headless tests for the refactored CreateBuildScreen (Mac-safe; no GPU/vLLM). (~1812 tok)
- `test_deployment_composer.py` — test_agent_composes_docker_deployment_draft_for_tui, test_agent_composes_generic_docker_with_fresh_c (~15884 tok)
- `test_docker_supervisor.py` — test_docker_supervisor_writes_scrubbed_run_stderr_to_log, test_docker_supervisor_scrubs_container_lo (~3536 tok)
- `test_docs.py` — test_tui_doc_matches_bindings, test_troubleshooting_doc_covers_every_remediation_kind, test_troubles (~3340 tok)
...
- `test_remediation.py` — test_agent_not_installed_remediation_names_bootstrap_command, test_agent_unreachable_remediation_nam (~882 tok)
- `test_remote_workflow.py` — test_remote_validation_uses_textual_smoke_for_real_config, test_remote_validation_checks_backend_evi (~14538 tok)
- `test_rpc_framing.py` — Tests: ndjson_frame_round_trips_json_object, ndjson_frame_rejects_oversized_payload, ndjson_frame_rejects_non_object_payload, protocol_stream_reade... (~10127 tok)
...
- `test_targets.py` — test_missing_targets_file_still_yields_implicit_local_target, test_targets_registry_loads_ssh_target (~1952 tok)
...
- `test_tui_smoke.py` — RecordingConfigAgent: test_tui_import_defaults_to_truecolor_for_figma_palette, test_tui_import_honor (~210446 tok)
...
- `ready-with-secret.log` (~22 tok)

## tests/fixtures/vllm_logs/

- `current-success.log` (~160 tok)
- `gated-model.log` (~58 tok)
- `hf-cache-miss.log` (~109 tok)
- `oom.log` (~91 tok)
- `port-in-use.log` (~56 tok)
