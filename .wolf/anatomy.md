...
> Auto-maintained by OpenWolf. Last scanned: 2026-06-09T08:26:53.397Z
...
- `fake_child.py` — Handler: do_GET, log_message, main, print_help + 1 more (~980 tok)
...
- `local.py` — URL configuration (~54592 tok)
...
- `schema.py` — URL configuration (~2119 tok)
- `targets.py` — URL configuration (~2030 tok)

## src/vela/engine/

- `__init__.py` — Engine, process, log, and vLLM-version adapter code. (~17 tok)
- `build_registry.py` — URL configuration (~11242 tok)
- `command_builder.py` — CommandBuildResult: build_command, is_local_model_reference, mask_preview_value, render_preview + 1 more (~2511 tok)
...
- `pin_model.py` — PinModelScreen: compose, on_mount, on_input_submitted, action_submit + 1 more (~1739 tok)
...
- `.DS_Store` (~1640 tok)
- `conftest.py` — config_dir, write_yaml, valid_config_text, clear_config_env (~386 tok)
...
- `test_command_builder.py` — Tests: exact_argv_env_for_serve_entrypoint, revision_pin_is_emitted_for_standalone_model_handoff, docker_error_classifier_covers_named_runtime_fail... (~4939 tok)
