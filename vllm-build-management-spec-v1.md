...
**Feature:** managed, selectable, independently-runnable vLLM builds · **Status:** spec-ready · **Audience:** the engineer(s) extending `vela`.
...
- CLI parity (`vela build …`) so everything is scriptable headlessly.
...
```
   ...
   └───────────────┘  └──────────────────────────────────────────────┘  └──────┬───────┘
                            ...
                            ▼ ~/.local/share/vela/builds/<id>/           ▼ vllm child
...
```
$XDG_DATA_HOME/vela/                 # default ~/.local/share/vela  (DATA, not state)
└── builds/
...
```json
{
  ...
  "paths": {                                // root absolute; rest relative → relocatable
    "root": "/home/u/.local/share/vela/builds/01J9...0001",
...
```bash
#!/usr/bin/env bash
# Generated by vela. Standalone — does NOT require the TUI.
# build_id: 01J9...0001  label: vllm-nightly-cu130-nvfp4
set -euo pipefail
BUILD_ROOT="/home/u/.local/share/vela/builds/01J9...0001"
...
{
  "build_id": "01J9...0001",
  "executable": "/home/u/.local/share/vela/builds/01J9...0001/bin/vllm",
  "python":     "/home/u/.local/share/vela/builds/01J9...0001/bin/python",
  "env_overlay": {
    "VIRTUAL_ENV": "/home/u/.local/share/vela/builds/01J9...0001/venv",
    "PATH_PREPEND": "/home/u/.local/share/vela/builds/01J9...0001/venv/bin"
  },
...
```
┌ Vela ─────  ▣ vllm-nightly ●  llama-3.1-70b  ●READY  http://127.0.0.1:8000  12:42:07 ┐
...
```
vela build add  --method nightly --channel cu130 --label nvfp4 [--python 3.12] [--env TORCH_CUDA_ARCH_LIST=10.0]
vela build add  --method commit  --commit <sha> --channel cu130
vela build add  --method git     --url <repo> --ref <sha> [--precompiled]
vela build add  --method pip     --spec 'vllm==0.11.0'
vela build add  --method wheel   --path /path/vllm-…whl
vela build adopt --path /opt/venvs/vllm-nightly [--copy]
vela build list [--json]
vela build select <id|label>            # sets global default (active.json)
vela build inspect <id|label>
vela build verify  <id|label>
vela build remove  <id|label> [--yes]   # refuses if in use
vela build run     <id|label> -- serve <model> [flags…]   # standalone, bypasses configs
```

`build add` streams to the terminal (same scrubbed sink) and exits non-zero on failure. Config discovery and run-artifact dirs are unchanged (canonical §14); builds live under `~/.local/share/vela/builds`.
